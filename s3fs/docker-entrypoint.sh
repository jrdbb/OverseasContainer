...
if [ "$IAM_ROLE" == "none" ]; then
  ...
  /usr/bin/s3fs  ${S3_BUCKET} ${MNT_POINT} -d -f -o url=${S3_URL},allow_other,retries=5,use_cache=/var/cache/s3fs,cache_size=1G
  echo 'started...'
else
  echo 'IAM_ROLE is set - using it to mount S3'
  /usr/bin/s3fs ${S3_BUCKET} ${MNT_POINT} -d -f -o url=${S3_URL},iam_role=${IAM_ROLE},allow_other,retries=5,use_cache=/var/cache/s3fs,cache_size=1G
fi